import { ExecutionMode, RulesetVersionStatus } from "@prisma/client";
...
function sendEtagMismatch(res: express.Response, etag: string, resource: Record<string, unknown>) {
  res.status(412).json({
    error: "ETAG_MISMATCH",
...
function assertIfMatchHeader(req: express.Request, res: express.Response, etag: string, resource: Record<string, unknown>) {
  const ifMatch = stripWeakEtag(req.header("if-match"));
  if (!ifMatch || ifMatch !== etag) {
    sendEtagMismatch(res, etag, resource);
    return false;
  }
  return true;
}
...
app.patch("/v1/rules/:ruleId", async (req, res) => {
  try {
    ...
    if (!assertIfMatchHeader(req, res, currentEtag, { ruleId: current.ruleId, name: current.name, lastUpdatedAt: current.lastUpdatedAt })) return;
...
app.patch("/v1/rule-versions/:ruleVersionId", async (req, res) => {
  try {
    ...
    if (!assertIfMatchHeader(req, res, currentEtag, { ruleVersionId: current.ruleVersionId, status: current.status })) return;
...
app.patch("/v1/ruleset-versions/:rulesetVersionId", async (req, res) => {
  try {
    ...
    if (!assertIfMatchHeader(req, res, etag, { rulesetVersionId: current.rulesetVersionId, status: current.status })) return;
...
app.post("/v1/ruleset-versions/:rulesetVersionId/entries:bulk", async (req, res) => {
  try {
    const rv = await prisma.rulesetVersion.findUnique({ where: { rulesetVersionId: req.params.rulesetVersionId } });
    if (!rv) throw new ApiError(404, "ruleset version not found");
    const serverEtag = getRulesetVersionEtag(rv);
    if (stripWeakEtag(req.body.ifMatch) !== serverEtag) {
      sendEtagMismatch(res, serverEtag, { rulesetVersionId: rv.rulesetVersionId, status: rv.status });
      return;
    }

    const user = (req as typeof req & { user?: { email?: string } }).user;
    const result = await bulkMutateRulesetEntries(prisma, {
      rulesetVersionId: req.params.rulesetVersionId,
      actor: user?.email ?? "unknown",
      operation: req.body.operation,
      entries: req.body.entries ?? []
    });

    res.setHeader("ETag", result.updatedRulesetVersionEtag);
    res.json(result);
  } catch (error) {
    handleError(res, error);
  }
...
app.post(/^\/v1\/ruleset-versions\/([^/]+):approve$/, async (req, res) => {
  try {
    ...
    if (!current) throw new ApiError(404, "ruleset version not found");
    if (!assertIfMatchHeader(req, res, getRulesetVersionEtag(current), { rulesetVersionId: current.rulesetVersionId, status: current.status })) return;
...
app.post(/^\/v1\/rulesets\/([^/]+):activateVersion$/, async (req, res) => {
  try {
    ...
    if (!assertIfMatchHeader(req, res, getRulesetVersionEtag(target), { rulesetVersionId: target.rulesetVersionId, status: target.status })) return;
...
app.post("/v1/ruleset-versions/:rulesetVersionId/entries", async (req, res) => {
  try {
    ...
    if (!assertIfMatchHeader(req, res, getRulesetVersionEtag(parent), { rulesetVersionId: parent.rulesetVersionId, status: parent.status })) return;
...
app.patch("/v1/ruleset-entries/:entryId", async (req, res) => {
  try {
    ...
    if (!assertIfMatchHeader(req, res, getRulesetEntryEtag(current), { entryId: current.entryId, enabled: current.enabled, orderPriority: current.orderPriority })) return;
...
app.get("/v1/ruleset-table", async (req, res) => {
  try {
    const data = await getRulesetTable(prisma, {
      ...
      status: req.query["filters[status]"] as RulesetVersionStatus | undefined,
      executionMode: req.query["filters[executionMode]"] as ExecutionMode | undefined,
...
app.get("/v1/audit-events", async (req, res) => {
  try {
    const items = await getAuditEvents(prisma, {
